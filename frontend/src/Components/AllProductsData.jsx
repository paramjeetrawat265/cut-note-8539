...
const AllProductsData = ({colorScheme}) => {
  ...
  const handleDelete = () => {
    if (product_id) {
      ...
      dispatch(productDelete(payload, headers))
        .then((res) => {
      ...
      <table>
        ...
        <tbody>
          {dashProducts?.map((ele, index) => (
            <tr
              className={index % 2 === 0 ? "userDetailsb" : "userDetailsw"}
              key={index}
            >
              <td>
                <img
              ...
              <td>{ele.quantity}</td>
              <td>
                <span onClick={() => handleOpen(ele._id)}>
                  <DeleteIcon />
                </span>
              </td>
            </tr>
