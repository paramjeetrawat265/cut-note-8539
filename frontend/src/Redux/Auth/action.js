...
const userSignup = (payload) => (dispatch) => {
  ...
  return axios
    .post(`${REACT_APP_API_URL}signup/`, payload)
    .then((res) => {
...
const adminAuthentication = (payload) => (dispatch) => {
  dispatch({type: types.USER_LOGIN_REQUEST});
  return axios
    .post(`${REACT_APP_API_URL}member/`, payload)
    .then((res) => {
      return dispatch({type: types.USER_LOGIN_SUCCESS, payload: res.data});
    })
    .catch((err) => {
...
const dashUserDelete = (payload, headers) => (dispatch) => {
  ...
  return axios
    .delete(`${REACT_APP_API_URL}dashboard/delete/user/${payload.id}`, headers)
    .then((res) => {
...
const dashUserData = (payload) => (dispatch) => {
  ...
  return axios
    .get(`${REACT_APP_API_URL}dashboard/users`, payload)
    .then((res) => {
      if (!res.data.status) {
